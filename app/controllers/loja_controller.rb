...
include HTTParty
  before_action :set_veiculo, except: [ :index, :pagamento, :confirma_pagto ] 
  before_action :set_cliente_cookie
  after_action :capturar_destino
  before_action :authenticate_user!, only: %i[ aluguel locacao pagamento confirma_pagto ]
  ...
  def confirma_pagto
    ...
    response = HTTParty.post("https://ws.sandbox.pagseguro.uol.com.br/v2/transactions?email=#{email}&token=#{token_loja}&", 
      body: body,
      headers: {
      ...
      if response.code = 200
        debugger
        reserva.update(status: Reserva::status[:retirada_pendente])
        redirect_to :pag_confirmado
      else
      render :pagamento, notice: "Revise os sados do cartão"
    end
  end
